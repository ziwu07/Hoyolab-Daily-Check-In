...
def Claim(act_id: str, cookie: dict):
    ...
    if (
        ...
        and str(resp["message"]) == "You've already checked in today, Trailblazer~"
    ):
        return True
    elif (
        str(resp["retcode"]) == "0"
...
def api_call(act_id: str, cookie: dict):
    headers = {
        ...
        "Referer": f"https://act.hoyolab.com/bbs/event/signin/hkrpg/index.html?act_id={act_id}",
    }
    try:
        response = requests.post(
            f"https://sg-public-api.hoyolab.com/event/luna/os/sign?lang=en-us&act_id={act_id}",
            headers=headers,
            cookies=cookie,
        )
